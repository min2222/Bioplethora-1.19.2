...
public class ShachathEntityStrikeGoal extends Goal {
    ...
    public void tick() {
        ...
        if (target.distanceToSqr(this.shachath) < 4096.0D && shachath.hasLineOfSight(target)) {
    ...
    public boolean doFilter(LivingEntity entity) {
        return entity != shachath && EntityUtils.IsNotPet(shachath).test(entity) && EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(entity) && shachath.hasLineOfSight(entity);
    }

    public void attackTarget(Level world, LivingEntity target, int phase) {
        float damageReduction = target.getUseItem().getItem() instanceof ShieldItem ? 2.25F : 1F;
        double d0 = -Mth.sin(shachath.yRot * ((float)Math.PI / 180F)) * 6;
        double d1 = Mth.cos(shachath.yRot * ((float)Math.PI / 180F)) * 6;
        ...
        if (target.getUseItem().getItem() instanceof ShieldItem) {
            ...
            if (target instanceof Player) {
                if (!((Player) target).getAbilities().instabuild) {
    ...
    public void ballParticleOnTarget(Level world, LivingEntity target) {
        int pSize = 10;
        for(int i = -pSize; i <= pSize; ++i) {
            for(int j = -pSize; j <= pSize; ++j) {
                for(int k = -pSize; k <= pSize; ++k) {
                    ...
                    double d6 = (double)Math.sqrt(d3 * d3 + d4 * d4 + d5 * d5) / 1.2 + shachath.getRandom().nextGaussian() * 0.05D;
