...
public class FrostbiteGolemRangedAttackGoal extends Goal {
    ...
    public void tick() {
        ...
        Level world = this.frostbite_golem.level;
        Vec3 vector3d = this.frostbite_golem.getViewVector(1.0F);
        ...
        if (livingentity.distanceToSqr(this.frostbite_golem) < 4096.0D && this.frostbite_golem.hasLineOfSight(livingentity)) {
            ++this.chargeTime;
            if (this.chargeTime == 10) {
                world.playSound(null, blockPos, SoundEvents.BEACON_ACTIVATE, SoundSource.HOSTILE, (float) 1, (float) 1);
            }

            if (!BPConfig.COMMON.hellMode.get()) {
                if (this.chargeTime == 20) {
                    world.playSound(null, blockPos, SoundEvents.SHULKER_SHOOT, SoundSource.HOSTILE, (float) 1, (float) 1);
